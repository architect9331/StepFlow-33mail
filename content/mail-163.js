// content/mail-163.js — Content script for 163 Mail (step 4)
...
function getLatestMailSubject() {
  const firstItem = findMailItems()[0];
  if (!firstItem) return '';
  const subjectEl = firstItem.querySelector('span.da0');
  const subject = subjectEl ? subjectEl.textContent : '';
  const ariaLabel = firstItem.getAttribute('aria-label') || '';
  return `${subject} ${ariaLabel}`.trim();
}
